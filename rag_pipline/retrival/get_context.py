...
class RetrievalPipeline:
    def __init__(self):
        ...
        # 1. Enhance Query
        enhanced_query = self.query_enhancer.enhance_query(user_query, session_id)
...
class RetrievalPipeline:
    def __init__(self):
        ...
        # 4. Context Orchestration (Iterative SQL)
        final_context = self.orchestrator.orchestrate(user_query, top_docs, session_id, max_iterations=10)
        print(f"Final context size: {len(final_context)}")
        
        return final_context

# Helper function for easy import
def get_context(user_query: str, user_id: str, session_id: str) -> List[Dict[str, Any]]:
    pipeline = ContextRetrievalPipeline()
    return pipeline.get_context(user_query, user_id, session_id)
